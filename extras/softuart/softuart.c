...
bool softuart_put(uint8_t uart_no, char c) {
	...
	for (uint8_t i = 0; i < 8; i++) {
		...
		uint8_t val = chbit(c, 1 << i);
		gpio_write(uart->tx_pin, val);
	}
	...
	while ((0x7FFFFFFF & sdk_system_get_time()) < (start_time + (uart->bit_time * 10))) {
		//If system timer overflow, escape from while loop
		if ((0x7FFFFFFF & sdk_system_get_time()) < start_time) {
			break;
		}
	}
	//sdk_os_delay_us(uart->bit_time * 1);
...
/*
 void Softuart_Putchar(uint8_t uart_no, char data)
 {
 unsigned i;
